...
namespace AgarioProject
{
    public partial class Form1 : Form
    {
            ...
            Task.Run(() =>
            {
                while (client.Connected)
                {
                    try
                    {
                        ...
                        {
                            // Deserialize and update player states
                            List<PlayerInfo> players = JsonConvert.DeserializeObject<List<PlayerInfo>>(message);
                            UpdatePlayerStates(players); // Update the UI
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void UpdatePlayerStates(List<PlayerInfo> players)
        {
            ...
            // Clear previous player PictureBoxes (excluding the current client player)
            ...
            foreach (PlayerInfo player in players)
            {
                ...
                // Create new PictureBox for the other players
